...
public class MyServerSocket2 {
	...
	public MyServerSocket2() {
		try {
			serverSocket = new ServerSocket(10000); // 서버소켓 객체 생성
			System.out.println("클라이언트 요청 대기 중...");
			socket = serverSocket.accept();// 데몬(리스닝)
			System.out.println("요청이 성공함");

			reader = // 읽기만 하는용도
					new BufferedReader(new InputStreamReader(socket.getInputStream())); // socket에서 읽어올겄이니 inputstream
	...
	class WriteThreas implements Runnable { // 내부클래스로 만든 이유 = socket 에 접근하기 위해서
